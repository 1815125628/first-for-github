...
//}
//static用法
//1.修饰局部变量,影响了变量的生命周期，使得变量出作用域时没有被销毁，本质上static修饰局部变量时改变了变量的存储位置
// 将数据从栈区放入静态区
// void test()//void不需要任何返回即不会有任何返回数值，在只输出如printf时使用
//{
//	//static int a = 1;//2，3，4.....11,即使用上一次运行后的a的值
//	int a = 1;//十个二
//	a++;
//	printf("%d ", a);
//	return 0;
...
//	return 0;
//}
//2.修饰全局变量时，全局变量的外部链接属性变成了内部链接属性，其他源文件(.c)就不能再使用这个全局变量了,即作用域不是整个工程了
//extern int a;
//int main()
//{
//	printf("%d\n",a);
...
//表达式3.5+1/2的计算结果是3.5因为1/2默认为0
//组成C程序的基本单位是函数，其组成部分包括函数头和函数体
//模块化程序设计的设计原则是高内聚和低耦合
//C语言是一种高级语言，用C语言编写的代码程序经过编译程序编译之后，生成一个后缀为OBJ的二进制文件(称为目标文件)
// 随后还要由“连接程序”软件，把该OBJ文件与C语言提供的各种库函数连接在一起，生成一个以EXE为后缀的可执行文件，才可以执行
